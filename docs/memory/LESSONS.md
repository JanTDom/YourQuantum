...
### L-022: Zmiana stojąca w sprzeczności z obowiązującym DEC wymaga uprzedniej rewizji DEC lub nie może zostać wykonana
W commicie `70c2f64` dekompozytor scenariuszy (`scenario_decomposer.py`) ustawił `is_accepted=True` dla przesłanek generowanych przez model (`llm_suggested`), aby użytkownik po zadaniu pytania otrzymywał od razu policzony rozkład zamiast pustego ekranu wyboru. Zmiana ta stała w bezpośredniej sprzeczności z obowiązującą decyzją architektoniczną DEC-032 (status ACTIVE: „Przesłanki z oznaczeniem llm_suggested nie wchodzą do obliczeń, dopóki użytkownik jawnie ich nie zatwierdzi”), z docstringiem modułu oraz z tekstem widocznym w interfejsie (`RecommendationView.tsx`: „Propozycje modelu (🤖) nie wchodzą do obliczeń, dopóki ich nie zatwierdzisz”). W rezultacie niezatwierdzone przez człowieka liczby modelu wchodziły do rozkładu. Zasada na przyszłość: Żadna optymalizacja UX ani chęć uniknięcia pustego ekranu startowego nie może naruszać aktywnej decyzji architektonicznej ani omijać bramek weryfikacyjnych. Zmiana stojąca w sprzeczności z obowiązującym DEC wymaga najpierw formalnej zmiany tego DEC przez Jana – dopóki to nie nastąpi, zmiana jest bezwzględnie niedozwolona. Prawidłowym rozwiązaniem problemu pustego ekranu jest czytelny stan zerowy (rozkład równomierny ze wskazaniem braku zatwierdzonych przesłanek) oraz ergonomia jednego kliknięcia („Zatwierdź wszystkie propozycje modelu”).

### L-023: Raport z wykonania opisuje wyłącznie to, co da się odczytać z kodu; cytaty z UI i nazwy metod muszą być weryfikowane u źródła
W sekcji 4 dokumentu `docs/REPORT_V6.md` doszło do błędu fabrykowania opisu interfejsu i metody obliczeniowej: opisano interfejs odtworzony z pamięci i intencji projektowych zamiast ze stanu faktycznego kodu źródłowego. Zacytowano nieistniejące w kodzie komunikaty („Odrzuć wszystkie”, „Oczekuje na weryfikację”, „Sugerowana przez AI”, „rozkład jednostajny”, „Wariant dominujący”), zmyślony „Główny baner ostrzegawczy (Amber Notification Banner)”, nieistniejące nazwy funkcji pomocniczych (`handleAcceptAllPremises`, `computeDynamicScenarioWeights`) oraz nazwano algorytm ważenia „bayesowskim”, podczas gdy kod w rzeczywistości wykonuje ważoną agregację przesłanek z rozkładem softmax/Gibbsa (`weighted_softmax_aggregation`). Jest to błąd z tej samej rodziny co L-021: generowanie przekonująco brzmiącego, lecz niezgodnego z faktami opisu. Zasada na przyszłość: Raport z wykonania zadania stanowi formalny ślad audytowy i musi opisywać wyłącznie to, co da się jednoznacznie odczytać z kodu. Każdy cytat z interfejsu w dokumencie musi być skopiowany dosłownie z pliku źródłowego wraz ze wskazaniem numeru linii, każda nazwa funkcji i zmiennej musi zostać zweryfikowana grepem w kodzie, a nazwa metody obliczeniowej w dokumentacji musi być ściśle zgodna z algorytmem realizowanym przez kod.
